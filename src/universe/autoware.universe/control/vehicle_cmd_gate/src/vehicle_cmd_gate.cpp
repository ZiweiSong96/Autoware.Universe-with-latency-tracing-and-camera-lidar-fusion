...
void VehicleCmdGate::publishControlCommands(const Commands & commands)
{
  ...
  // ofstream of ("/home/mlabszw/autoware_with_caret/my_evaluate/end_time.txt",ios::app);
...
void VehicleCmdGate::publishStatus()
{
  ...
  auto end_time = steady_clock_.now();
  streambuf* coutBuf = std::cout.rdbuf();
  ofstream of ("/home/mlabszw/autoware_with_caret/my_evaluate/operation_mode_end_time.txt",ios::app);
  streambuf* fileBuf = of.rdbuf();
  std::cout.rdbuf(fileBuf);
  std::cout<<fixed<<setprecision(10)<<end_time.seconds()<<" ";
  std::cout<<end_time.seconds()<<std::endl;
