...
void ImageDiagNode::ImageChecker(const sensor_msgs::msg::Image::ConstSharedPtr input_image_msg)
{
  ...
  ofstream of ("/home/mlabszw/autoware_with_caret/my_evaluate/Traffic_light/ImageChecker_start_time.txt",ios::app);
...
void ImageDiagNode::shiftImage(cv::Mat & img)
{
  ...
  auto start_time = steady_clock_.now();
  streambuf* coutBuf = std::cout.rdbuf();
  ofstream of ("/home/mlabszw/autoware_with_caret/my_evaluate/Traffic_light/ImageShift_start_time.txt",ios::app);
  streambuf* fileBuf = of.rdbuf();
  std::cout.rdbuf(fileBuf);
  std::cout<<fixed<<setprecision(10)<<start_time.seconds()<<" ";
  std::cout<<start_time.seconds()<<std::endl;
